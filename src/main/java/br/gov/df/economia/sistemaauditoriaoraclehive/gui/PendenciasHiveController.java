...
import br.gov.df.economia.sistemaauditoriaoraclehive.model.entities.PendenciasHive;
import br.gov.df.economia.sistemaauditoriaoraclehive.db.HiveDecDatabaseOperations;
import br.gov.df.economia.sistemaauditoriaoraclehive.gui.util.ComboBoxUtil;
import br.gov.df.economia.sistemaauditoriaoraclehive.model.entities.DuplicidadeId;
...
public class PendenciasHiveController {
    ...
    private <T> void configureColumn(TableColumn<PendenciasHive, T> column, Function<PendenciasHive, T> valueExtractor) {
    ...
    private void executeQueryAndUpdateUI() {
        if (hiveDatabaseOperations != null) {
            ...
            List<PendenciasHive> resultList = hiveDatabaseOperations.getQueryResultsDet();
            // Converter a lista em ObservableList
            ObservableList<PendenciasHive> observableResultList = FXCollections.observableArrayList(resultList);
